 ...
 * successful HealthCareApp build. The result of the script is the generation of a build list text
 ...
 * HealthCareApp build.groovy script.
 *
 * If the --buildHash option is omitted or if no previous successful HealthCareApp build exists with a
 * buildHash property, the contents of HealthCareApp/build/files.txt will be copied to ${workDir}/buildList.txt
 * resulting in a full build.
 *
 * usage:  impacts.groovy [options]
 *
 *  **NOTE - HealthCareApp/build/build.properties will be loaded (if exists) at script startup.
...
// if buildHash argument omitted, then just copy HealthCareApp/build/files.txt to buildlist and exit
...
// if no lastBuildHash, then just copy HealthCareApp/build/files.txt to buildlist and exit
